...
export const NumberButton: FC<NumberButtonProps> = ({
  ...
  return (
    <Button
      ...
      className={cn(
        "relative aspect-square h-auto text-lg font-semibold p-1 flex flex-col items-center justify-center leading-none",
    ...
    >
      <span className="text-4xl sm:text-5xl">{animalEmoji}</span>
      <span className="absolute top-1 left-1 font-bold text-base bg-black/40 text-white rounded-full h-6 w-6 flex items-center justify-center">
        {number}
      </span>
      <span className="text-[10px] sm:text-xs uppercase font-bold tracking-tighter mt-0.5">{animalName}</span>
      {countInSelection > 0 && (
        <span
          className="absolute -top-2 -right-2 bg-accent text-accent-foreground text-sm font-bold rounded-full h-6 w-6 flex items-center justify-center leading-none shadow-lg"

...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const fetchUserData = async () => {
      if (firebaseUser) {
        ...
        try {
          ...
          toast({ title: "Erro de Permissão", description: "Não foi possível carregar os dados do seu usuário.", variant: "destructive" });
        } finally {
  ...
  }, [firebaseUser, authLoading, db, toast]);

  const updateUserBalance = useCallback((newBalance: number) => {
    setCurrentUser(prevUser => {
        if (prevUser && prevUser.saldo !== newBalance) {
            return { ...prevUser, saldo: newBalance };
        }
        return prevUser;
    });
  }, []);
  ...
  const value = { currentUser, firebaseUser, login, signInWithGoogle, logout, register, isLoading, isAuthenticated, updateUserBalance };
